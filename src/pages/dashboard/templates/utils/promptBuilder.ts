...
- **DESAIN RESPONSIF & BEBAS OVERFLOW SELULER (RESPONSIVE & MOBILE-FIRST DESIGN - SANGAT PENTING)**:
  * Semua kontainer, seksi, kartu, flexbox, dan grid **WAJIB** dirancang responsif secara penuh (mobile-first).
  * **DILARANG KERAS** menggunakan lebar piksel tetap/statis yang besar (seperti \`w-[400px]\`, \`w-[500px]\`, \`min-w-[450px]\`, \`w-[600px]\`) tanpa breakpoint responsive. Selalu gunakan persentase atau batas maksimal lebar (seperti \`w-full max-w-sm\`, \`max-w-md\`, \`max-w-lg\`, \`w-11/12\`) agar konten termuat dengan sempurna di layar HP sempit (lebar 360px - 410px).
  * Pastikan kartu mempelai pria dan wanita memiliki margin dan padding yang mengecil secara elegan di layar seluler (gunakan \`p-4 sm:p-8\`, \`mx-3 sm:mx-6\`).
  * Nama mempelai kustomer bisa sangat panjang. Oleh karena itu, **DILARANG** menggunakan \`whitespace-nowrap\` pada nama mempelai (\`groom_name\`, \`bride_name\`), melainkan biarkan teks melipat ke bawah secara otomatis (\`break-words\` atau default wrap). Gunakan ukuran font yang fluid/fleksibel (seperti \`text-3xl sm:text-4xl md:text-5xl\`) agar nama mempelai tidak terpotong pada layar HP sempit.
